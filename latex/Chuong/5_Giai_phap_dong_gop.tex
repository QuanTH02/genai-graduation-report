...
\begin{enumerate}
    ...
    \begin{figure}[H]
        \centering
        \includegraphics[width=0.9\linewidth]{Hinhve/gh_at_1.png}
        \caption{Tạo PAT Github}
        \label{fig:gh_at_1}
    \end{figure}
    
    \item Thiết lập Secrets Repository
    ...
    \begin{figure}[H]
        \centering
        \includegraphics[width=0.9\linewidth]{Hinhve/gh_at_2.png}
        \caption{Thiết lập Secrets trên GitHub}
        \label{fig:gh_at_2}
    \end{figure}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.9\linewidth]{Hinhve/gh_at_3.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.7\linewidth]{Hinhve/gh_at_4.png}
...
Việc xác định một phương pháp gợi ý tối ưu đóng vai trò then chốt và đầy thử thách trong quá trình xây dựng ứng dụng đề xuất điện ảnh. Sau khi nghiên cứu các công trình khoa học và tài liệu chuyên ngành, đồng thời tiến hành thực nghiệm để so sánh hiệu quả giữa các mô hình khác nhau, tôi đã quyết định lựa chọn thuật toán Content-based làm giải pháp chủ đạo cho dự án này.

\textbf{Giới thiệu thuật toán Content-based:}

Trong hệ thống lọc dựa trên nội dung này, tôi thiết lập một hồ sơ đặc trưng riêng biệt cho từng tác phẩm điện ảnh dựa trên các thuộc tính nội tại của chúng. Về mặt toán học, hồ sơ này được mô hình hóa thành một vectơ đặc trưng, đóng vai trò là cơ sở dữ liệu đầu vào để tính toán độ tương đồng giữa các bộ phim.

\begin{itemize}
    \item Movie name: Tiêu đề của tác phẩm điện ảnh
    \item Genres: Phân loại thể loại mà tác phẩm thuộc về
    \item Describe: Nội dung tóm tắt về cốt truyện của tác phẩm
    \item Cast: Danh sách các nghệ sĩ tham gia diễn xuất trong tác phẩm
    \item Director: Người đảm nhiệm vai trò chỉ đạo sản xuất tác phẩm
\end{itemize}

\textbf{Phân tích, giải pháp:}

Content-based là phương thức gợi ý dựa trên việc đề xuất các đối tượng có những đặc tính tương đồng với đối tượng mà người dùng đã lựa chọn trước đó. Do vậy, trong phạm vi bài toán này, tôi đã thực hiện tính toán mức độ tương quan giữa các tác phẩm điện ảnh dựa trên tập hợp các vectơ đặc trưng đã xác định ở trên.
...
Trong đó:
\begin{itemize}
  \item \(A\) và \(B\) là hai vector biểu diễn cho hai tác phẩm điện ảnh hoặc hai đối tượng người dùng.
  \item \(A \cdot B\) là giá trị tích vô hướng được tính từ hai vector \(A\) và \(B\).
  \item \(\|A\|\) và \(\|B\|\) tương ứng là giá trị chuẩn (norm) của vector \(A\) và vector \(B\).
\end{itemize}

\textbf{Kiểm tra tính chính xác:}

Để đánh giá độ tin cậy của mô hình sau khi áp dụng công thức tính toán độ tương đồng, tôi tiến hành quy trình kiểm chứng bằng cách đo lường mức độ tương quan trên từng thuộc tính độc lập như tên phim, thể loại, dàn diễn viên, đạo diễn và phần mô tả. Cuối cùng, tôi thực hiện đối chiếu kết quả tổng hợp với các chỉ số riêng lẻ này để xác nhận tính nhất quán và độ chính xác của thuật toán.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.9\linewidth]{Hinhve/rcm_ct_kq_1.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.9\linewidth]{Hinhve/rcm_ct_kq_2.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.9\linewidth]{Hinhve/rcm_ct_kq_3.png}
